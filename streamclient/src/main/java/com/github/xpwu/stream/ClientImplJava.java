...
class ClientImplJava {
  ...
  private void asyncConnectSuc(ClientJava.ConnectHandler handler) {
    new Handler().post(new Runnable() {
      @Override
      public void run() {
        handler.onSuccess();
  ...
  private void asyncSendSuc(ClientJava.ResponseHandler handler, byte[] response) {
    new Handler().post(new Runnable() {
      @Override
      public void run() {
  ...
  private void runWaiting(Error error) {
    for(ClientJava.ConnectHandler handler: waitingConnects) {
      if (error == null) {
  ...
  // 无论当前连接状态，都可以重复调用
  public void connect(ClientJava.ConnectHandler handler) {
    ...
    this.waitingConnects.add(new ClientJava.ConnectHandler() {
      @Override
  ...
  // 如果没有连接成功，直接返回失败
  public void send(byte[] data, Map<String, String> headers, ClientJava.ResponseHandler handler) {
...
//          Log.d("ClientImpl", "onError: ", error);
  ...
  public OptionJava.Value config;
